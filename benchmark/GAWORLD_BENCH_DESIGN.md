...
> ⚠️ **口径（2026-09-25 增补）**：`data/citymap.md` 覆盖的是杭州滨江一带约 19×15 km 的
> **一个片区**，而下表的通勤类锚点是**全杭州口径**。实测：该片区能复现全市的「5 公里
> 以内通勤比重」（52.3% vs 52%），但通勤距离均值无论怎么标定最多到 5.94 km，够不着全市
> 的 8.1 km——19 公里的图上不存在那条尾巴。因此 `commute_minutes` 与 `transit_share`
> 已在 `gaworld_bench.py` 中标为 `scope: city_wide`，**只报告不打分**。拿片区去比全市，
> 量的是口径差不是模型误差。详见 `docs/proposals/2026-09-19-endogenous-road-congestion.md` §15。

| 指标 | 真实锚点 | 口径 | 来源 | 仿真数据源 | 误差度量 | 容差 |
|------|---------|------|-----------|---------|------|
| 恩格尔系数 | **28.8%**（城镇 2024；全国 29.8%，农村 32.3%） | 城镇 ✅计分 | 国家统计局 2024 国民经济和社会发展统计公报 | `economy/wealth_snapshot.csv:engel_coefficient` 均值 | 相对误差 | 0.15 |
| 居民储蓄率 | **~35%**（口径敏感，区间 30–43%） | 城镇 ✅计分 | 见 §6 局限 | `economy/wealth_snapshot.csv:savings_rate` 均值 | 相对误差 | 0.30 |
| 平均通勤时耗 | **34.5 min**（杭州单程 2024，备用 32.9） | 全市 ⚠️不计分 | 2024 年度中国主要城市通勤监测报告（中规院×百度地图） | 通勤记忆 `avg_travel_time` | 相对误差 | 0.25 |
| 公交机动化分担率 | **47.6%**（杭州；绿色出行 >70%） | 全市 ⚠️不计分 | 杭州市交通运输局 2024 | `state` 出行方式计数 | 相对误差 | 0.25 |
