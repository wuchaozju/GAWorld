...
- **`gaworld/city/knowledge.py`：城市的产业与就业画像**，创建城市时联网搜集
  （搜索 → LLM 结构化），失败则退回 OSM 地图统计，再失败返回空画像。
  **产业名是自由文本**（「纺织业」「跨境电商」），不绑 `JOB_INDUSTRY_MAP` 的六分类——
  一座城的性格恰恰活在那六类抹掉的区别里；只有落到经济模块时才由 `industry_mix()` 映射。
- **`gaworld/city/news.py`：两个时钟。** 抓取按**真实时间** TTL 门控，
  投喂按**仿真时间**轮转。仿真 365 天可能一下午跑完，「每仿真日拉一次新闻」
  等于在新闻没变的同一个下午发 365 次搜索。实测 365 仿真日 = **2 次搜索**。
- **四条影响通道**（`gaworld/city/context.py` 统一出口）：
  - **提示词** → `background`，居民知道自己住在什么样的城市；
  - **技能成长** → `derive_growth_profile` / `evolve_growth_profile`，
    旅游城市的居民更可能去学导游；
  - **就业收入** → `economy.macro.industry_conditions`，**没有改经济模块一行代码**，
    该字段本来就从配置读，城市包只是提供覆盖；
  - **城市级 RAG** → 写进每位居民的 `external_info`，复用已有检索路径，
    不需要平行的城市级查询。
- 职业动机与社交传染**分成两条采纳路径**：因为政府推旅游而学导游是一次职业决策
  （`kind=skill`、`category=职业`、`career_link=True`），不是从朋友那染上的爱好；
  默认概率是社交传染的一半，城市经济按月推着人走、不会一夜换岗。
- CLI `knowledge` / `news` 子命令，`/api/city/{knowledge,news}` 读写接口，
  以及「城市」页签的知识库面板（产业占比、趋势、收入系数、来源链接）。
- `tests/test_city_knowledge.py`（35 条）：护栏与通道各占一半。

### Fixed

- **`profile_signature` 会让第二座城市的影响永远不出现。** 它是成长画像的缓存键，
  原本只含 job/personality/daily_life/values。城市开始参与推导后，
  同一个居民在纺织城和旅游城会共用一份缓存——第二座城的 city_hint 进了提示词，
  但结果直接从缓存返回。现在城市签名并进该键。
- **地图兜底会把标注密度伪装成经济结论。** 绍兴的 OSM 包里只有 55 个住宅区 +
  22 所学校，按比例算得出「教育业 100%」，并据此给出 `education=1.25` 的收入系数。
  现在要求至少 2 个产业类别、6 个节点，不够就返回空画像，四条通道全部静默。
- **`industry_conditions` 原本全部顶到上限。** 相对 1/6 均匀分布缩放，
  而真实城市只有两三类产业、占比必然远高于 1/6，于是每个都被钳到 1.4，
  趋势项永远动不了任何东西。改为按占比线性映射到 `[1-span, 1+span]`。
- **趋势被自身权重稀释。** 趋势原本乘以产业权重，而权重已经由占比项承担了，
  等于重复计算——一个占比很大的衰退产业算出来只比稳定产业差一点点。
  改为按权重归一化的**方向**（[-1,1]）。

## [Unreleased] — 2026-09-12 — 城市：给一个地名，造一座城

### Added

- **`gaworld/city/`：从地名创建整座可仿真城市**（CLI `python -m gaworld.city`、
  Python API、控制台「城市」页签）。流水线每一层都能降级，因为
  「没有网络」和「这个村子 OSM 上没有」都必须仍然给出一座能跑的城：
  地理编码（Nominatim）失败 → 合成地点记录；真实地图（Overpass/OSM）失败 → 只用程序化地图；
  程序化地图**始终生成**（兜底 + LLM 提示词上下文）。
- **城市是独立的包**，落在 `data/cities/<slug>/`：`city.json` 清单 + `citymap.md`
  + `map.geojson` + `environment.json` + `agents.csv` + `profiles.md`。
  注册表是目录扫描而非索引文件——索引会在有人手工拷贝或删除文件夹时悄悄和现实脱节。
  非 ASCII 地名的 slug 保留原字符（`data/cities/绍兴柯桥/`）：
  没有不引依赖就能做的音译，退回哈希会让整个目录不可读。
- **环境按地点推导，不是一套全局默认**。气候取自 `|lat|` 决定自然事件池，
  社会事件按规模选池（村里是「集市开集」，大城市才有「地铁运营延误」）。
  **`background` 提示词也会被改写**——默认 background 写着「中国·杭州」，
  不换掉的话新城市里的智能体会一边住在柯桥、一边以为自己在杭州。
- **三条加 agent 的路径**：批量合成（复用 `gaworld.population`，
  `district_weights` 换成该城真实存在的行政区）、追加单个、把已有 agent 迁入。
  三者都写成仿真器原生可读的 CSV + profile MD，无转换层。
- **`config["city"]` 切换整个世界**：`map_path` / `csv_path` / `md_path` / `map_mode`、
  环境事件表与 `background` 整体指向该城市包。优先级为
  `dashboard_config.json` → 全局 `environment_config.json` → **城市包** → `GAWORLD_CONFIG_OVERRIDES`。
  城市排在全局环境文件**之后**是刻意的：那个文件是默认值而非针对某座城的意图，
  否则大陆性气候的城市会继承全局文件里的亚热带事件。
  配置里写了一座已删除的城市不会让仿真起不来，解析失败即退回默认世界。
- `tests/test_city_{bundle,create,agents,api}.py`（67 条）。
  加 agent 的测试全部让产物**往返 `parse_profile`**，而不是相信写入端。

### Added（续）

- **主控制台运行控件新增「城市」下拉**：不用先去「城市」页签，直接选好城市再点运行。
  与 CLI `use` 和城市页签写的是同一个开关（`dashboard_config.json` 的 `"city"`），
  不是第二套机制。下拉显示 `城市名 · N 人 · 真实地图/程序化`，
  **没有居民的城市禁选**——它跑不起来，与其选完再被服务端拒绝，不如不给选。
- **Agent IDs 越界会在运行前拦下。** 编号是每座城市各自从 1 开始的，
  配置里留着 `agent_ids: [37]` 再切到一座 30 人的城，`build_agent` 会对空结果取
  `.iloc[0]`，几分钟后在日志里抛一个光秃秃的 pandas `IndexError`——那时操作者早就不看了。
  现在直接返回 400：「城市「梅湾村」只有 30 位居民，但 Agent IDs 里有 [37]」。
  未知 slug 同样拒绝而非静默写入——`apply_city` 解析失败会退回默认世界，
  等于这次运行悄悄换了个地方发生。

- **「城市」页签新增地图预览**（`GET /api/city/map` + `site/dashboard/city.{html,js,css}`）。
  用的是控制台地图面板和仿真回放**同一个渲染器**（`citymap-view.js`），
  滚轮缩放 / 拖拽平移 / 双击复位都由它自带，没有再造一套。
  预览永远显示这座城**实际会用的那张图**（有真实 OSM 包就是真实图），
  不会出现"预览一张、跑另一张"。服务端按 `(路径, mtime)` 缓存，重新生成城市自动失效；
  响应里剥掉 `geojson`（渲染器不读，留着体积翻倍）。
  地图放在**宽的左栏**而不是 380px 的详情侧栏——174 个地点在侧栏里糊成一团。

### Fixed

- **两座城市共用一套记忆和一个世界时钟。** `memory_dir` / `log_dir` 以及另外 13 个
  运行期目录都是全局的 `output/*`，而 `agent_ids` 是按城市编号的——于是 A 城的 1 号
  和 B 城的 1 号共用同一份 `agent_1.json`、同一个 `sim_state.json`。换城市不是换世界，
  是让新居民继承上一座城的人生。现在选了城市会把**整棵运行树**指向
  `output/cities/<slug>/`：记忆、日志、向量库、日记、图表、经济账本、可视化 trace、
  生活事件、协作会话全在里面（映射表见 `gaworld/city/config.py` 的 `RUN_PATHS`）。
  目录结构与默认的 `output/` **逐项对齐**不是为了好看：Analytics 是把 `state/`、
  `economy/` 拼到运行根目录上读的，`replay_runs` 是靠 glob `output/*/visualization`
  找 trace 的，改名字这两处会无声地失效。运行根目录放在 `output/` 而不是城市包里，
  是因为城市包是这座城的**定义**，应当能直接拷给别人，不该捎带某个人二十年的记忆。
  没选城市时一切照旧（还是 `output/memory`），旧数据不动。
  顺带把 `state_output_dir` / `network_output_dir` / `agent_import_output_dir`
  三个键正式写进默认配置——它们本来只以"读取处的兜底值"存在，没声明的键既没法被
  城市重定向，也不会出现在设置面板里。
- **删城市会留下它的记忆。** 删掉 `data/cities/<slug>/` 后 `output/cities/<slug>/`
  还在，重新建一座同名的城，新居民会直接继承被删掉那座城的记忆和世界时钟
  ——正是城市隔离要消灭的那个 bug，只是延后发生。现在 `delete_city` 一并删除运行目录，
  且沿用原有的越界保护（拒绝删除注册表之外的路径时，运行目录也不动）。
- **从没跑过的智能体一开局就在 2047 年。** `sim_state.json` 只有一个全局
  `last_day`，续跑时不管选的是谁，都从它往下加。于是换一批 agent_ids（换城市尤其如此）
  跑出来的新居民，直接继承了别人攒下的 7832 天——开局日期配的是 2026-09，
  他醒来却是 2048-02。现在按 agent 记录天数（`sim_state.json` 的 `agent_last_day`）：
  **只有这次运行里真正跑过的 agent 才计入续跑**，一个都没跑过就从 Day 1 重新起算；
  混跑时取其中跑得最远的那个。旧的 state 文件没有 `agent_last_day`，
  迁移时把 `output/memory/` 里**所有**已有记忆的 agent 都按全局 `last_day` 补上
  ——只补本次运行的那几个，会让其他 agent 下次续跑时被错误地重置回 Day 1。
- **批量添加居民时，填什么数字都被浏览器拒绝。** `popSize` 写的是
  `min="1" step="10"`——HTML 的步进基准是 `min`，于是合法值变成 1/11/21/…，
  100、200、50 全被判非法，**连它自己的默认值 100 都是非法的**，
  提示「请输入有效值。两个最接近的有效值分别为91和101」。
  改成 `step="1"` 并补上 `max="5000"`；越界由服务端裁剪并通过
  `city.pop_clamped` 明说（「请求 5 人，采样器范围为 20–5000」），不闷声改掉。
  同样的问题在「新建城市」的 `citySize` 上也存在（55、137 会被拒），一并修掉。
  已扫描全站其余 number 输入，无同类问题。
- **「批量添加居民」的输入框标签写着「初始人口」。** 它和「新建城市」共用了
  `city.field_size` 这一个 key，而这个表单是往**已有**城市里追加，不是初始化。
  拆出 `city.field_add_size`（新增人数 / Residents to add）。
- **真实地图没有河流时会被画上一条钱塘江。** OSM 包里找不到河流时，
  `_real_river_from_lnglat` 退回示例河流，于是绍兴（包里 0 条河）被画上
  **2794 个水面 tile（占 16%）和 15 座凭空的桥**——水面挡路、桥进了路网判定，
  `real_city_map_text` 还告诉 LLM 这座城有条它从来没有的河。
  现在真实地图的"没找到"就是"没有"；`_river_polyline` 也区分
  **显式空 path**（没有河）与**缺 key**（回退示例），`or` 会把两者合并掉。
  虚拟地图保留默认（那本来就是编的），`data/citymap.md` 行为不变。
- **村庄会凭空长出地铁。** 地图层在没有 `@metro` 时回退到示例线路，
  进而污染 `choose_transport_mode` 与票价。现在：用了 `@` 指令的地图就是在
  明确声明自己的交通，没写就是真没有；完全没有 `@` 指令的旧地图仍吃默认值
  （`data/citymap.md` 与 `data/testcitymap.md` 行为逐字节不变）。
- **非杭州纬度的东西向距离偏差 +31%。** 投影常量按 30°N 标定，
  而经度度数随 `cos(lat)` 收缩。真实地图包现在可带 `meta.origin` 投影锚点，
  正反投影都遵循它（巴黎纬度实测 31.1% → 0.0%）。
  没有 `meta.origin` 的包（`data/hangzhou_real.geojson`）行为完全不变。
- **迁入的 agent 档案仍自称住在原城市。** 语料有两种写法——
  手写的是「居住西湖区。」，生成的是「居住于余杭·商品房。」——字面替换两种都匹配不上。
  改用 `parse_profile` 同款正则重写。
- **地理编码失败后仍去抓合成 bbox 的 OSM**，白白耗一轮重试。
- **一次建城要几十分钟。** 公共 Overpass 镜像经常一个个地不可达，
  而代码每个分类都重新轮询死镜像。现在粘住已知可用的镜像，并给整次抓取
  一个 wall-clock 预算——`urlopen` 的 timeout 是**每次 socket 操作**的，
  慢速返回的镜像能远远跑过它，只有整体 deadline 才真正兜得住。
- **行政区 bbox 吞掉邻市。** 柯桥区的官方 bbox 跨 66 km，
  会把诸暨的枫桥镇、店口镇一并拉进来，居民通勤 70 km。
  改为按地点规模收敛 bbox（柯桥区 → 28.9 km）。
- **被 Ctrl-C 打断的建城会挡住重试。** 没有 `city.json` 的目录是上次中断的残骸，
  不是一座城，现在直接覆盖，不需要 `--force`。
- `real_city_map_text` 的提示词表头硬编码「真实杭州地图」，改为读 `meta.city`。

### Docs

- 新增[城市教程](docs/CITY_TUTORIAL.md)；[地图模式](docs/MAP_MODES.md)补上
  `meta.origin` 投影锚点与地铁默认值的变化；README（中英）、`AGENTS.md`、
  `docs/FEATURES.md`、控制台文档索引同步。

## [Unreleased] — 2026-09-11 — 工作台：大五人格设置面板

### Added

- **智能体工作台第 2 步新增「大五人格」面板**（`site/dashboard/studio.{html,js,css}`
  + `GET/POST /api/agents/<id>/big5`）。五个 z 分滑块、两端行为锚点、
  段落并排显示、逐维一致性标记。写入 `data/agents_big5.csv`，
  与工作台其他种子编辑一致——**下次运行生效**（插件在 `agents.built` 读一次，运行时只读）。
- **和上面九个状态变量分成两块，不混排。** 分数是先采样、再据此写出段落的；
  当初给散文打分只会把状态变量读回来（开放性与 `mobility_intent` 相关 0.90）。
  把两者呈现成同一族，等于邀请操作者用手把那次独立性撤销掉。
- **一致性标记是分数的算术，不是对段落做文本分析。**
  生成器写进段落的维度当且仅当 |z| ≥ 0.5，据此比较「段落是从哪组分数写出来的」与「现在是多少」：
  `与段落相反`（变号，必然矛盾）／`与段落有出入`／`段落里没有`／`段落多写了`。
  **刻意不做关键词匹配**——这份语料上关键词探针已经误导过一次
  （E 探针读出 −0.13，因为词表用了两端都出现的主题名词），
  而一个自信的错指示比没有更糟：操作者会信它而不去读段落。
- **`authored_z` 快照列**：第一次手改时把「段落所依据的那组分数」存下来。
  没有它，基线会被第一次编辑毁掉，冲突提示在刷新后就消失了——
  而这正是面板要防的那件事。插件的 `load_profiles` 用 DictReader，多一列不受影响（有测试钉住）。
- 手改后 `source` 置为 `hand_edited`（不再冒充采样器的产物），
  并清空 `redundant`（那是共线性闸门对**已经变掉的**那组值下的判断）。
- `tests/test_dashboard_big_five.py`（15 条）：读写往返、裁剪、非法输入、
  四种一致性标记、基线跨刷新存活、以及插件仍能读新列。

### Fixed

- **`test_daily_routine_context` 那两条的真正病因找到了：是日历，不是玄学。**
  `generate_daily_routine` 在模拟的 Day 3 落在周六/周日时会**再发一次**
  「周末个性化日程改写器」的调用，而测试的捕获函数用的是赋值而非 setdefault，
  于是断言拿到的是周末那条提示词。仿真日历默认从真实日期起算，
  所以**每 7 天里有 2 天**会红。
  **我先后误判过两次**——先当成 RNG 顺序依赖，后当成 `output/` 缓存状态；
  都不是。改成 setdefault，并加一条测试分别用工作日与周末起点各跑一遍，
  钉住「这个提示词不该随今天是星期几而变」。
